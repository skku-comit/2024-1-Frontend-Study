...
const classpacket = document.getElementById('classpacket');
...
data.forEach((item) => {
  ...
  classbox.appendChild(newImage);

  // 제목 추가
  const newTitle = document.createElement('h3');
  newTitle.textContent = item.title;
  classbox.appendChild(newTitle);

  const badgepacket = document.createElement('div');
  badgepacket.classList.add('badge');

  //badge에 난이도, 스택, 캠퍼스 추가
  const newLevel = document.createElement('div');
  newLevel.textContent = item.level;
  badgepacket.appendChild(newLevel);

  const newStack = document.createElement('div');
  newStack.textContent = item.stack[0];
  badgepacket.appendChild(newStack);

  const newCampus = document.createElement('div');
  newCampus.textContent = item.campus;
  badgepacket.appendChild(newCampus);

  // classpacket에 classbox 추가
  classpacket.appendChild(classbox);
  classbox.appendChild(badgepacket);
});
